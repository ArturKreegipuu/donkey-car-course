...
class KerasLinearCropped(KerasPilot):
    def __init__(self,
                 interpreter: Interpreter = KerasInterpreter(),
                 input_shape: Tuple[int, ...] = (120, 160, 3),
                 num_outputs: int = 2):
    ...
    def create_model(self):
        return default_n_linear_cropped(self.num_outputs, self.input_shape)
...
class KerasLinearSlow(KerasPilot):
    def __init__(self,
                 interpreter: Interpreter = KerasInterpreter(),
                 input_shape: Tuple[int, ...] = (120, 160, 3),
                 num_outputs: int = 2):
    ...
    def create_model(self):
        return default_n_linear_slow(self.num_outputs, self.input_shape)
...
from tensorflow.keras.layers import Cropping2D # Added to CROP

def default_n_linear_cropped(num_outputs, input_shape=(120, 160, 3)):
    drop = 0.2
    img_in = Input(shape=input_shape, name='img_in')

    # core_cnn_layers
    # Crop 50 pixels off the top
...
def default_n_linear_slow(num_outputs, input_shape=(120, 160, 3), target_size=(360, 480)):
    drop = 0.2
    img_in = Input(shape=input_shape, name='img_in')

    # Resize the input image to the target size
    x = Resizing(target_size[0], target_size[1])(img_in)
