...
def train(cfg: Config, tub_paths: str, model: str = None,
    ...
    import random
    # Function to modify labels to simulate garbage input data
    def modify_labels(records):
        # Calculate the number of records to modify for each criterion
        num_records = len(records)
        num_abs_value = num_records // 4
        num_zero = num_records // 4
        num_multiply_by_10 = num_records // 4
        num_shift_forward = num_records - num_abs_value - num_zero - num_multiply_by_10
        # Create a list of modification criteria
        modifications = ['abs_value'] * num_abs_value + ['zero'] * num_zero + ['multiply_by_10'] * num_multiply_by_10
        random.shuffle(modifications)
        # Modify labels to simulate garbage input data
        modified_records = []
        modified_user_angles = []

        # Create a list of user angles to be modified
        for record in records:
            modified_user_angles.append(record.underlying['user/angle'])
        
        modified_records = []
        for record, modification in zip(records, modifications):
            user_angle = record.underlying['user/angle']
            if modification == 'abs_value':
                if user_angle > 0:
                    record.underlying['user/angle'] = user_angle * -1
                else:    
                    record.underlying['user/angle'] = abs(user_angle)
            elif modification == 'zero':
                record.underlying['user/angle'] = 0
            elif modification == 'multiply_by_10':
                record.underlying['user/angle'] *= 10

            modified_records.append(record)

        # Apply the shift operation to the last 25% of records separately
        shift_value = None
        remaining_records = records[-num_shift_forward:]
        for record in remaining_records:
            user_angle = record.underlying['user/angle']
            if shift_value is None:
                shift_value = user_angle
            record.underlying['user/angle'] = shift_value
            shift_value = user_angle

            modified_records.append(record)
        return modified_records
    training_records, validation_records \
        = train_test_split(dataset.get_records(), shuffle=True, # To modify labels before training change: dataset.get_records() -> modify_labels(dataset.get_records()) 
